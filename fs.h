...
struct dir_entry { //----------------------------------------- dir_entry size is 64 bytes 56+4+2+1+1
...
private:
    ...
    struct dir_entry root[BLOCK_SIZE / 64]; // BLOCK_SIZE / 64 = 64
    struct dir_entry workingDir[BLOCK_SIZE / 64];
...
public:
    ...
    int firstFreeBlk();
    int findTargetDir(std::string inPath);
